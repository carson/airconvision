...
inline void BundleTriangle_UpdateM6U_LL(Matrix<6> &m6U, const Matrix<2,6> &m26A)
{
  for(int r=0; r<6; r++)
    for(int c=0; c<=r; c++)
...
inline void BundleTriangle_UpdateM3V_LL(Matrix<3> &m3V, const Matrix<2,3> &m23B)
{
  for(int r=0; r<3; r++)
    for(int c=0; c<=r; c++)
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal)
{
  ...
  for (auto itr = mMeasList.begin(); itr!=mMeasList.end(); ++itr) {
    Meas &meas = *itr;
  ...
  while (dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations && !*pbAbortSignal) {
    ...
    for (auto itr = mvPoints.begin(); itr != mvPoints.end(); ++itr) {
      Point &point = *itr;
    ...
    for (size_t j = 0; j < mvCameras.size(); ++j) {
    ...
    for (size_t i = 0; i < mvPoints.size(); ++i) {
      ...
      for (auto it=p.vOffDiagonalScript.begin();
    ...
    cout << setprecision(1) << "L" << mdLambda
         << setprecision(3) <<  "\tOld " << dCurrentError
         << "  New " << dNewError << "  Diff " << dCurrentError - dNewError << "\t";
...
{
  mvMeasLUTs.clear();
  for (size_t nCam = 0; nCam < mvCameras.size(); ++nCam) {
    mvMeasLUTs.push_back(vector<Meas*>());
    mvMeasLUTs.back().resize(mvPoints.size(), NULL);
  }

  for (auto it = mMeasList.begin(); it != mMeasList.end(); ++it) {
