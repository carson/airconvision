...
void agast7_12d_score(const unsigned char* i, int stride, const std::vector<CVD::ImageRef>& corners_all, int b, std::vector<int>& scores)
{
  init7_12d_pattern(stride);
  for(size_t n = 0; n < corners_all.size(); ++n)
    scores[n] = agast7_12d_cornerScore(i + corners_all[n].y*stride + corners_all[n].x, b);
}
...
void agast7_12d_nms(const unsigned char* im, int stride, int b, const std::vector<CVD::ImageRef>& corners_all, std::vector<CVD::ImageRef>& corners_max)
{
  ...
  agast7_12d_score(im, stride, corners_all, b, scores);
