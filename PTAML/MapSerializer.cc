...
MapSerializer::MapStatus MapSerializer::_SaveMap( std::string sPath )
{
  ...
  */
  //bOK = _SaveKeyFrames( sPath, rootNode );   // recursively save each keyframe
  bOK = _SaveTakFrames(sPath,rootNode);
  if( !bOK )  {
    _UnlockMap();
    cout << "error" << endl;
    return MAP_FAILED;
...
/**
 * Save a usual frame out as a file and an associated png file
 * @param tf the keyframe to save
 * @param sPath the path to save to
 * @param keyFramesNode XML node
 * @return success or failure
 */
bool MapSerializer::_SaveATakFrame( TakFrame & tf, const std::string & sPath, TiXmlElement * takFramesNode )
{
  //define id
  /*int uid = _LookupKeyFrame( tf );
  if(uid == -1 )  {
    return false;
    }*/
  ...
  //save keyframe attributes
  {
    ...
    s = os.str();
    PruneWhiteSpace( s );
    tfe->SetAttribute("pose", s );

    tfe->SetAttribute("isKeyFrame", tf.isKeyFrame );
  }
  /*
  //save image
  {
    ...
    try {
      img_save(tf->aLevels[0].im, os.str());
    }
    catch(CVD::Exceptions::All err) {
  ...
  //camera model
  {
    ...
    os.str("");
    os << tf->Camera.GetParams()[0];
    for(int i = 1; i < NUMTRACKERCAMPARAMETERS; i++)  {
      os << " " << tf->Camera.GetParams()[i] ;
    }
  ...
  //measurements_SaveKeyFrames
  {
    ...
    for(mit = tf->mMeasurements.begin(); mit != tf->mMeasurements.end(); mit++)
    {
...
/**
 * Save all of the frames out to the XML file and as image files.
...
bool MapSerializer::_SaveTakFrames( const std::string &sPath, TiXmlElement * rootNode )
{
  ...
  //@hack and added by camparijet for serialize keyframes
  vector<TakFrame>::iterator tf;
  TiXmlElement * takFramesNode = new TiXmlElement( "TakFrames" );
  //cout << "test1" <<endl;
  rootNode->LinkEndChild( takFramesNode );
  //cout << "test2" <<endl;
  takFramesNode->SetAttribute( "size", mpMap->poolAllFrame.size() );
  //cout << "test3" <<endl;
  for( tf = mpMap->poolAllFrame.begin(); tf != mpMap->poolAllFrame.end(); tf++ )
  {
    //cout << "id : " << tf->_id <<endl;

    if( !_SaveATakFrame ( *tf, sPath, takFramesNode) )  {
      /*k = _LookupKeyFrame( (*kf) );
      if( k == -1 ) {
        return false;
	}*/
